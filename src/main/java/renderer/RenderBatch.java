...
public class RenderBatch {
    ...
    private final int TEX_COORDS_OFFSET = COLOR_OFFSET + COLOR_SIZE * Float.BYTES; //looking for offset which is where tex coords start
    private final int TEX_ID_OFFSET = TEX_COORDS_OFFSET + TEX_COORDS_SIZE * Float.BYTES;
    private final int VERTEX_SIZE = 9; //9 floats in a vertex
    ...
    public void addSprite(SpriteRenderer spr){
        ...
        int index = this.numSprites; //adds to end of array
        this.sprites[index] = spr;
        this.numSprites ++;

        if (spr.getTexture() != null){
            if (!textures.contains(spr.getTexture())){ //makes sure we don't already have texture in list
        ...
        if (numSprites >= this.maxBatchSize){ //filled up array of sprites
            this.hasRoom = false;
        }
    }
    ...
    public void render(){
        ...
        for(int i = 0; i <textures.size(); i++){ //goes through all textures
            glActiveTexture(GL_TEXTURE0 + i + 1); //activates texture in appropriate slot
            textures.get(i).bind(); //binds texture to slot
        }
